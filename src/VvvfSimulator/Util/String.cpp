...
namespace VvvfSimulator::Util::String
{
	...
	QString TranslatableFmtString::makeUntrString() const
	{
		...
		for (const auto &arg : args)
		{
			if (const auto x = std::get_if<QString>(&arg.a); x)
				...
				str = str.arg(
				std::get<TranslatableFmtString>(arg.a).makeUntrString(),
				arg.fieldWidth, arg.fillChar
			);
		}
	...
	)
	{
		...
		{
			if (!doExcepts) return s;
			QStringList words = s.split(QRegularExpression(
				QString::fromRawData(wordRegEx.data(), wordRegEx.size())
			), Qt::SkipEmptyParts);
			for (auto &word : words) handleExceptCase(word);
		...
		else
		{
			...
			for (auto &word : words)
			{
				if (!doExcepts || !handleExceptCase(word)) // Not an exception, follow regular flow
				{
					switch (cases)
					{
					case ChangeCases::ToLower:
